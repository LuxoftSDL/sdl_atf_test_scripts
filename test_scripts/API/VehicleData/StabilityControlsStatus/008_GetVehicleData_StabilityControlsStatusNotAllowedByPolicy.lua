...
--
-- Steps:
-- 1) App sends GetVehicleData in (NONE, FULL, LIMITED, BACKGRAUND) level (with stabilityControlsStatus = true) request to SDL
--    SDL not sends VehicleInfo.GetVehicleData (with stabilityControlsStatus = true) request to HMI
...
common.Step("Send get vehicle data, FULL level", common.processGetVDunsuccess,
  { "stabilityControlsStatus", resultCode })
common.Step("Set HMI Level to Limited", common.hmiLeveltoLimited, { 1, "MAIN" })
runner.Step("Send get vehicle data, LIMITED level", common.processGetVDunsuccess,
  { "stabilityControlsStatus", resultCode })
runner.Step("Set HMI Level to BACKGROUND", common.hmiLeveltoBackground)
runner.Step("Send get vehicle data, BACKGROUND level", common.processGetVDunsuccess,
  { "stabilityControlsStatus", resultCode })
