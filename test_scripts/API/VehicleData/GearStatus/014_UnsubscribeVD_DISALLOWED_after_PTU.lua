...
-- Preconditions:
-- 1) RPCs(Subscribe/UnsubscribeVehicleData) with the `gearStatus` param exists in app's assigned policies.
-- 2) App sends valid RPCs(Subscribe/UnsubscribeVehicleData) requests with gearStatus=true to the SDL
...
-- 3) App re-sends UnsubscribeVehicleData request with gearStatus=true to the SDL.
-- SDL does:
