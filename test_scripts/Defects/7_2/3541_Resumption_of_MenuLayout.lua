---------------------------------------------------------------------------------------------------
-- https://github.com/smartdevicelink/sdl_core/issues/3541
---------------------------------------------------------------------------------------------------
-- Steps:
...
local function sendWindowCapabilities()
  ...
  onSysCaps.appID = common.getHMIAppId()
  common.getHMIConnection():SendNotification("BasicCommunication.OnSystemCapabilityUpdated", onSysCaps)
end
...
local function sendAddSubMenu()
  ...
  dataToHMI.appID = common.getHMIAppId()
  local cid = common.getMobileSession():SendRPC("AddSubMenu", asmParams)
  common.getHMIConnection():ExpectRequest("UI.AddSubMenu", dataToHMI)
  :Do(function(_, data)
      common.getHMIConnection():SendResponse(data.id, data.method, "SUCCESS", {})
    end)
  common.getMobileSession():ExpectResponse(cid, { success = true, resultCode = "SUCCESS" })
...
local function reRegisterApp()
  ...
  :Do(function()
    ...
    :Do(function()
        ...
        dataToHMIsgp.appID = common.getHMIAppId()
        common.getHMIConnection():ExpectRequest("UI.SetGlobalProperties", dataToHMIsgp)
        :Do(function(_, data)
        ...
        dataToHMIasm.appID = common.getHMIAppId()
        common.getHMIConnection():ExpectRequest("UI.AddSubMenu", dataToHMIasm)
        :Do(function(_, data)
            utils.printTable(data)
            common.getHMIConnection():SendResponse(data.id, data.method, "SUCCESS", {})
          end)
